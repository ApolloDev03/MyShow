...
export default function CityMasterPage() {
    ...
    const [confirmCityId, setConfirmCityId] = useState<number | null>(null);
    const [confirmCityName, setConfirmCityName] = useState<string>("");
    const [confirmStateName, setConfirmStateName] = useState<string>("");
    ...
    const fetchCitiesByState = async (sid: number) => {
        const res = await axios.post(`${apiUrl}/city-by-state`, { stateid: sid }, authConfig());
    ...
    // When state filter changes -> refresh (toast)
    useEffect(() => {
        (async () => {
            try {
                setLoading(true);
                ...
                console.error(e);
                toast.error(e?.message || "Failed to refresh cities.");
            } finally {
                setLoading(false);
            }
        })();
    ...
    // ===== CRUD =====
    const addCityApi = async (sid: number, name: string) => {
        const res = await axios.post(`${apiUrl}/city-add`, { stateid: sid, name }, authConfig());
        return res.data;
    };

    const updateCityApi = async (cityId: number, sid: number, name: string) => {
        const res = await axios.post(`${apiUrl}/city-update`, { city_id: cityId, stateid: sid, name }, authConfig());
        return res.data;
    };

    const deleteCityApi = async (cityId: number) => {
        const res = await axios.post(`${apiUrl}/city-delete`, { city_id: cityId }, authConfig());
        return res.data;
    };
    ...
    const saveCity = async () => {
        ...
        if (!sid) return toast.error("Please select state.");
        if (!name) return toast.error("Please enter city name.");
        ...
        if (dup) return toast.error("This city already exists in selected state.");
    ...
    // ✅ open confirm popup (no window.confirm)
    const askDelete = (c: CityRow) => {
        setConfirmCityId(c.cityId);
        setConfirmCityName(c.cityName);
        setConfirmStateName(c.stateName);
        setConfirmOpen(true);
    };
    ...
    const confirmDelete = async () => {
        if (!confirmCityId) return;

        try {
            setSaving(true);
            const data = await deleteCityApi(confirmCityId);
            if (!data?.success) throw new Error(data?.message || "City delete failed");
    ...
    return (
        <main className="min-h-screen bg-[#f6f7fb] text-black">
            ...
            <div className="mx-auto w-full max-w-7xl px-4 py-8">
                ...
                <section className="mt-6 rounded-2xl border border-black/10 bg-white p-4 shadow-sm">
                    <div className="grid gap-3 sm:grid-cols-12 sm:items-end">
                        ...
                        <div className="sm:col-span-12 flex items-center justify-between">
                            <p className="mt-2 text-xs text-black/60">{loading ? "Loading..." : `Total: ${cities.length}`}</p>
                        </div>
                    </div>
                </section>
            ...
            {/* ✅ Delete Confirm Popup (no window.confirm) */}
            {confirmOpen ? (
                <div
                    className="fixed inset-0 z-999 bg-black/40 p-4 sm:p-6 flex items-center justify-center"
                    onMouseDown={(e) => {
                    ...
                    <div className="w-full max-w-md overflow-hidden rounded-2xl border border-black/10 bg-white shadow-xl">
                        <div className="flex items-start justify-between gap-3 border-b border-black/10 px-5 py-4">
                            <div>
                                ...
                                <p className="text-xs text-black/50">This action cannot be undone.</p>
                            </div>
                            <button
                                type="button"
                        ...
                        <div className="px-5 py-5">
                            <p className="text-sm text-black/80">
                                ...
                                <span className="font-semibold">{confirmCityName}</span>
                                {confirmStateName ? (
                                    <>
                                        {" "}
                                        from <span className="font-semibold">{confirmStateName}</span>
                                    </>
                                ) : null}
                                ?
                            </p>
                        ...
                        <div className="flex items-center justify-end gap-3 border-t border-black/10 px-5 py-4">
                            ...
                            <button
                                ...
                                disabled={saving}
                                className="rounded-xl bg-red-600 px-5 py-2.5 text-sm font-semibold text-white hover:opacity-95 disabled:opacity-60"
                            >
