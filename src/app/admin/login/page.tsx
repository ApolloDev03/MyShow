...
export default function UserLoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (mobile.trim().length < 8) {
            const msg = "Please enter valid mobile number.";
            setError(msg);
            toast.error(msg);
            return;
        }
        ...
        try {
            ...
            if (res.status >= 200 && res.status < 300 && isSuccess(data) && data?.data?.token) {
                ...
                toast.update(toastId, {
                    render: message || "Login successful!",
                    type: "success",
                    isLoading: false,
                    autoClose: 1500,
                });
            ...
            const errMsg = message || `Login failed (${res.status}).`;
            setError(errMsg);
            toast.update(toastId, {
                render: errMsg,
                type: "error",
        ...
        <section className="min-h-screen bg-linear-to-br from-primary to-secondary text-black flex items-center justify-center px-4 py-10">
            ...
            <div className="w-full max-w-sm rounded-2xl bg-white backdrop-blur p-6 shadow-lg">
                ...
                <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
                    ...
                    {/* Optional: you can remove this error line if you want only toast */}
                    {error ? <p className="text-center text-sm text-red-600">{error}</p> : null}
